...
namespace SmartAuditor.Editor
{
    ...
    /// DiagnosticParams are saved in the Smart Auditor settings asset, which should be included in the project's version control repository.
    /// By default, every <see cref="AnalysisOptions"/> is constructed with a copy of the global DiagnosticParams for use during analysis.
    ...
    public sealed class DiagnosticParams : ISerializationCallbackReceiver
    {
        ...
        internal class PlatformParams
        {
            ...
            // Parallel storage for float-typed parameters. Kept separate from m_Params rather than
            // unified into a tagged-union dictionary because Unity SerializeField can't serialize
            // System.Object — two typed lists are the simplest persistence-friendly shape.
            [JsonProperty("floatParams")]
            Dictionary<string, float> m_FloatParams = new Dictionary<string, float>();

            internal int ParamsCount => ((m_Params == null) ? 0 : m_Params.Count)
                + ((m_FloatParams == null) ? 0 : m_FloatParams.Count);
            ...
            // Float counterpart of ParamKeyValue, persisted alongside the int list for Unity
            // serialization. New name to avoid colliding with the int serialized field above.
            [Serializable]
            internal struct FloatParamKeyValue
            {
                public string Key;
                public float Value;

                public FloatParamKeyValue(string key, float value)
                {
                    Key = key;
                    Value = value;
                }
            ...
            [JsonIgnore]
            [SerializeField]
            List<FloatParamKeyValue> m_SerializedFloatParams = new List<FloatParamKeyValue>();
            ...
            public PlatformParams(PlatformParams copyFrom) : this()
            {
                ...
                foreach (var key in copyFrom.m_FloatParams.Keys)
                {
                    SetFloatParameter(key, copyFrom.m_FloatParams[key]);
                }
            }
            ...
            public bool TryGetFloatParameter(string paramName, out float paramValue)
            {
                return m_FloatParams.TryGetValue(paramName, out paramValue);
            }

            public void SetFloatParameter(string paramName, float paramValue)
            {
                m_FloatParams[paramName] = paramValue;
            }
            ...
            public void PreSerialize()
            {
                ...
                m_SerializedFloatParams.Clear();
                foreach (var key in m_FloatParams.Keys)
                {
                    m_SerializedFloatParams.Add(new FloatParamKeyValue(key, m_FloatParams[key]));
                }
            }
            ...
            public void PostDeserialize()
            {
                ...
                m_FloatParams.Clear();
                foreach (var kvp in m_SerializedFloatParams)
                {
                    m_FloatParams[kvp.Key] = kvp.Value;
                }
                m_SerializedFloatParams.Clear();
            }
        ...
        /// <summary>
        /// Register a float-valued parameter by declaring its name and default value.
        /// </summary>
        public void RegisterParameter(string paramName, float defaultValue)
        {
            if (!m_ParamsStack[0].TryGetFloatParameter(paramName, out _))
            {
                m_ParamsStack[0].SetFloatParameter(paramName, defaultValue);
            }
        }
        ...
        /// <summary>
        /// Get the float value of a named parameter. The parameter must have previously been registered as a float.
        /// </summary>
        public float GetFloatParameter(string paramName)
        {
            ...
            if (CurrentParamsIndex > 0 && CurrentParamsIndex < m_ParamsStack.Count)
            {
                if (m_ParamsStack[CurrentParamsIndex].TryGetFloatParameter(paramName, out paramValue))
                    return paramValue;
            }

            if (m_ParamsStack[0].TryGetFloatParameter(paramName, out paramValue))
                return paramValue;

            throw new Exception($"Diagnostic float parameter '{paramName}' not found. Check that it is properly registered");
        }
        ...
        /// <summary>
        /// Set the value of a named float parameter for a given analysis platform.
        /// </summary>
        public void SetParameter(string paramName, float value, BuildTarget platform = BuildTarget.NoTarget)
        {
            foreach (var platformParams in m_ParamsStack)
            {
                if (platformParams.Platform == platform)
                {
                    platformParams.SetFloatParameter(paramName, value);
                    return;
                }
            }

            var newParams = new PlatformParams(platform);
            newParams.SetFloatParameter(paramName, value);
            m_ParamsStack.Add(newParams);
        }
        ...
        internal void RegisterParameters()
        {
            ...
            foreach (var type in TypeCache.GetTypesDerivedFrom(typeof(DiagnosticAnalyzer)))
            {
                if (type.IsAbstract)
                    continue;
